...
namespace RandomCharacterCreator
{
    class Program
    {
        ...
        private static void Main(string[] args)
        {
            while (true)
            {
                int level;
                Console.Write("Level: ");
                if (!Int32.TryParse(Console.ReadLine().Trim(), out level))
                {
                    level = 1;
                }

                var randomNumberForGender = rnd.Next(1, 2);

                Gender gender = (Gender)randomNumberForGender;
                ...
                if (nameType.GetProperty("Ethnicity") != null)
                {
                    name += $" ({nameType.GetProperty("Ethnicity").GetValue(nameInstance)})";
                }
